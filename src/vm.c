    ...
    if (arity >= 0)
      ...
      if (nargs < min_arity)
        ...
        /* Reminder for optional and keyword args list:
           ...
           Keyword lists are reprsented as follows.
           If the formal parameters used in the definition of the
           lambda are:

           (... :key (a 5 a) (b "b") c)

           Then the list will be represented itnernally in the
           ...
           Something similar is done with optionals, except that we
           don't keep the names of arguments. If the definition was

           (... :optional d (e 5) (f 10 f?))
        ...
        SCM keywords = CLOSURE_KEY_ARGS(func);
        SCM key = keywords;
        SCM ptr = rest;
        ...
        /* Optionals given by the user are over, so now we push
           ...
           optional list is an assoc list, we push the caar. */
        while (opt != STk_nil) {
            push(CAR(CAR(opt)));
            opt = CDR(opt);
        }
        ...
        int i=0;
        while (opt != STk_nil) {
            /* The test is cadar of opt, and we just need to check
               if cdar is nil or not. */
            if (CDR(CAR(opt)) != STk_nil) /* test was specified */
                push ((i < n_opts)
                      ? STk_true
                      : STk_false);
            i++;
        ...
        while (key != STk_nil) {
            push( STk_key_get(CAR(CAR(key)),        /* key name */
                              ptr,
